...
class PostViewSet(viewsets.ViewSet):
    ...
    def list_public_and_private_by_author(self, request, author_id=None):
        ...
        if request.user.is_authenticated:
            queryset_private = Post.objects.filter(
                author_id=author_id, visibility='private')
    ...
    def get__posts(self, request, user_id=None):
    ...
    def get_user_profile_posts(self, request, user_id=None):
        target_user = UserProfile.objects.filter(pk=user_id).first()
        if not target_user:
            return Response({'detail': 'User not found.'}, status=status.HTTP_404_NOT_FOUND)
        target_user_posts = Post.objects.filter(author=target_user)
        public_posts = target_user_posts.filter(visibility='public')

        if request.user.is_authenticated:
            is_mutual = UserFollow.objects.filter(
                follower=request.user, followed=target_user
            ).exists() and UserFollow.objects.filter(
                follower=target_user, followed=request.user
            ).exists()

            if is_mutual:
                private_posts = target_user_posts.filter(visibility='private')
                public_posts = public_posts | private_posts
